...
def region_repalce(word_index, model, texts, embeddings,vocab,word, content, alternative_words):
    ...
    def invisible_meta_replace(soup, meta, meta_words):
        if word in meta_words:
            ...
            twords = replace.B_reserve_order(embeddings, vocab, word, alternative_words)
    ...
    def similar_replace(soup,head, text):
        if word in text:
            ...
            tword = replace.B_reserve_order(embeddings, vocab, word, alternative_words)
    ...
    if soup.find_all("body"):
        ...
        hotword = replace.B_reserve_order(embeddings, vocab, word, alternative_words)
