...
class LGS_Attack(object):
    ...
    def perturb_one(self, x_cur, x_cur_score, x_orig, neigbhours, w_select_probs, ori_label_mask,ori_tag):
        ...
        if x_tag[0] != ori_tag[0] or x_tag[1] != ori_tag[1] or x_tag[2] != ori_tag[2]:  #
            # if np.argmax(new_x_preds[best_id]) != target:
            # print("改了", x_orig[rand_idx], x_new[rand_idx], x_tag, ori_tag)
            return [1, x_new]
    ...
    def attack(self,x_ori, perturbable_words_id, perturbable_words_substitution):
        ...
        ori_score,ori_rel,_,_ = self.predict(x_ori)
        self.query_count = 0
        ...
        print("ori_tag:", ori_tag, "label:",ori_label_mask,"target:", target, 'ori:', ori_score, "perturbable num", len(perturbable_words_id))
        ...
        for i in range(self.max_iters):
            ...
            pop_tags_all=[]
            for a in pop:
                pt,pt_tag,_,_ = self.predict(a)
                #pt_tag = self.parse_rel_list(pt_tag)
            ...
            for pt_id in range(len(pop_scores_all)):
                ...
                print("pre_tag:",pop_tags_all[pt_id][0],pop_tags_all[pt_id][1],pop_tags_all[pt_id][2])
                if pop_tags_all[pt_id][0]!=ori_tag[0] or pop_tags_all[pt_id][1]!=ori_tag[1] or pop_tags_all[pt_id][2]!=ori_tag[2]:
                    return pop[pt_id],self.query_count
                # if abs(pop_scores[pt_id]-ori_label_mask.sum())<1e-2 and pt.shape[0]==ori_label_mask.shape[0]:
    ...
    def get_hot_word(self, x_ori, perturbable_words_id, power):
        ...
        losses = self.calculate_conf(x_ori, perturbable_words_id, batch_ent_shaking_tag)
        ...
        return hotwordsid,hotwords

    def calculate_conf(self, x_ori, perturbable_words_id, pred):
