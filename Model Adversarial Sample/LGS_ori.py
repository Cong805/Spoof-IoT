        ...
        for i in range(x_len):
            if neighbours_len[i] == 0:
                ...
                if j < hotwordslen and i == hotwordsid[j]:
                    w_select_probs.append(neighbours_len[i] * 2) #* (power - j))
                    j += 1
                    continue
                if x_ori[i] in hotwords:
                    w_select_probs.append(neighbours_len[i] * 2) #* (20 - hotwords.index(x_ori[i])))
                    continue
        ...
        for i in range(self.max_iters):
            ...
            for id in range(len(pop)):
                ...
                if np.random.uniform() < p_change:
                    tem = self.perturb_one(x,pop_scores[id], x_ori, neigbhours_list, w_select_probs,ori_label_mask,ori_tag)
                    if tem is None:
                        return None,self.query_count
                    if tem[0] == 1:
