        ...
        for i in range(x_len):
            if neighbours_len[i] == 0:
                ...
                if j < hotwordslen and i == hotwordsid[j]:
                    w_select_probs.append(neighbours_len[i] * (power - j))
                    j += 1
                    continue
                if x_ori[i] in hotwords:
                    w_select_probs.append(neighbours_len[i] * (20 - hotwords.index(x_ori[i])))
                    continue
    ...
    def calculate_conf(self, x_ori, perturbable_words_id, pred,pred_head,pred_tail):
        ...
        for i in range(l):
            ...
            with torch.no_grad():
                ...
                pred_head_r0 = pred_head[:, 0, :].reshape(-1,1).view(-1)
                pred_head_r2 = pred_head[:, 2, :].reshape(-1,1).view(-1)
                #pred_head_r1 = pred_head[:, 2, :].reshape(-1, 1).view(-1)
                head_outputs_r0 = head_rel_shaking_outputs[:, 0, :, :]
                ...
                #head_outputs_r1 = head_rel_shaking_outputs[:, 2, :, :]
