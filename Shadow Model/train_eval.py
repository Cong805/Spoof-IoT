...
def test(config, model, test_iter, use_type):
    # test
    model.load_state_dict(torch.load('F:/final_iot1/iot/识别/模型工具/DL模型/Model_Based/dataset/saved_dict/v/TextCNN.ckpt',map_location=torch.device('cpu')))
    model.eval()
    start_time = time.time()
    if use_type == 'train':
        ...
        with torch.no_grad():
            for texts, labels in test_iter:
                ...
                a = torch.max(outputs.data, 1)[0].cpu().numpy()
                b = torch.nn.functional.softmax(outputs, dim=None, _stacklevel=3, dtype=None)
                torch.set_printoptions(threshold=float('inf'))
                c = torch.max(b.data, 1)[0].cpu().numpy()
                d = torch.max(b.data, 1)[1].cpu().numpy()
